use std::collections::HashMap;
use rand::prelude::SliceRandom;
use std::sync::Mutex;
use lazy_static::lazy_static;

lazy_static! {
    static ref TRANSPOSITION_TABLE: Mutex<HashMap<u64, f32>> = Mutex::new(HashMap::new());
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Direction {
    Up,
    Down,
    Left,
    Right,
}

#[derive(Debug, Clone)]
pub struct GameBoard {
    board: [[u32; 4]; 4],
    move_count: u32,
    // Optimized representation for faster operations
    empty_mask: u16,  // Bitmask of empty cells
    max_tile: u32,    // Cached max tile
}

impl GameBoard {
    pub fn new() -> Self {
        let mut board = [[0; 4]; 4];
        // add two initial tiles
        Self::add_random_tile(&mut board);
        Self::add_random_tile(&mut board);
        
        let empty_mask = Self::calculate_empty_mask(&board);
        let max_tile = Self::calculate_max_tile(&board);
        
        GameBoard {
            board,
            move_count: 0,
            empty_mask,
            max_tile,
        }
    }

    pub fn get_board(&self) -> [[u32; 4]; 4] {
        self.board
    }

    pub fn set_board(&mut self, board: [[u32; 4]; 4]) {
        self.board = board;
        self.empty_mask = Self::calculate_empty_mask(&board);
        self.max_tile = Self::calculate_max_tile(&board);
    }

    pub fn get_move_count(&self) -> u32 {
        self.move_count
    }

    // Optimized empty mask calculation
    fn calculate_empty_mask(board: &[[u32; 4]; 4]) -> u16 {
        let mut mask = 0u16;
        for i in 0..4 {
            for j in 0..4 {
                if board[i][j] == 0 {
                    mask |= 1 << (i * 4 + j);
                }
            }
        }
        mask
    }

    // Optimized max tile calculation
    fn calculate_max_tile(board: &[[u32; 4]; 4]) -> u32 {
        board.iter().flatten().max().copied().unwrap_or(0)
    }

    fn add_random_tile(board: &mut [[u32; 4]; 4]) {
        let mut empty_cells = Vec::new();
        for i in 0..4 {
            for j in 0..4 {
                if board[i][j] == 0 {
                    empty_cells.push((i, j));
                }
            }
        }
        
        if let Some((i, j)) = empty_cells.choose(&mut rand::thread_rng()) {
            board[*i][*j] = if rand::random::<f32>() < 0.9 { 2 } else { 4 };
        }
    }

    // Optimized board hash using Zobrist hashing
    fn board_hash(&self) -> u64 {
        let mut hash = 0u64;
        for i in 0..4 {
            for j in 0..4 {
                let value = self.board[i][j];
                if value != 0 {
                    // Use log2 of value for better hash distribution
                    let log_value = value.trailing_zeros() as u64;
                    hash ^= log_value << ((i * 4 + j) * 4);
                }
            }
        }
        hash
    }

    pub fn move_tiles(&mut self, direction: Direction) -> bool {
        let mut moved = false;
        let mut new_board = self.board;
        
        match direction {
            Direction::Left => {
                for row in 0..4 {
                    let (new_row, row_moved) = Self::merge_row(&self.board[row]);
                    new_board[row] = new_row;
                    moved |= row_moved;
                }
            }
            Direction::Right => {
                for row in 0..4 {
                    let mut reversed_row = self.board[row];
                    reversed_row.reverse();
                    let (merged_row, row_moved) = Self::merge_row(&reversed_row);
                    new_board[row] = {
                        let mut result = merged_row;
                        result.reverse();
                        result
                    };
                    moved |= row_moved;
                }
            }
            Direction::Up => {
                for col in 0..4 {
                    let column: [u32; 4] = [
                        self.board[0][col],
                        self.board[1][col],
                        self.board[2][col],
                        self.board[3][col],
                    ];
                    let (merged_col, col_moved) = Self::merge_row(&column);
                    for (row, &value) in merged_col.iter().enumerate() {
                        new_board[row][col] = value;
                    }
                    moved |= col_moved;
                }
            }
            Direction::Down => {
                for col in 0..4 {
                    let mut column: [u32; 4] = [
                        self.board[0][col],
                        self.board[1][col],
                        self.board[2][col],
                        self.board[3][col],
                    ];
                    column.reverse();
                    let (merged_col, col_moved) = Self::merge_row(&column);
                    let mut result_col = merged_col;
                    result_col.reverse();
                    for (row, &value) in result_col.iter().enumerate() {
                        new_board[row][col] = value;
                    }
                    moved |= col_moved;
                }
            }
        }

        if moved {
            self.board = new_board;
            self.move_count += 1;
            // Update cached values
            self.empty_mask = Self::calculate_empty_mask(&self.board);
            self.max_tile = Self::calculate_max_tile(&self.board);
        }
        
        moved
    }

    fn merge_row(row: &[u32; 4]) -> ([u32; 4], bool) {
        let mut new_row = [0; 4];
        let mut write_pos = 0;
        let mut i = 0;
        let mut moved = false;
        
        while i < 4 {
            if row[i] == 0 {
                i += 1;
                continue;
            }
            
            if i + 1 < 4 && row[i] == row[i + 1] && row[i] != 0 {
                new_row[write_pos] = row[i] * 2;
                write_pos += 1;
                i += 2;
                moved = true;
            } else {
                new_row[write_pos] = row[i];
                write_pos += 1;
                i += 1;
            }
        }
        
        // check if row changed
        if !moved {
            for i in 0..4 {
                if new_row[i] != row[i] {
                    moved = true;
                    break;
                }
            }
        }
        
        (new_row, moved)
    }

    pub fn is_game_over(&self) -> bool {
        // Fast check: if there are empty cells, game is not over
        if self.empty_mask != 0 {
            return false;
        }
        
        // check for possible merges
        for i in 0..4 {
            for j in 0..4 {
                let current = self.board[i][j];
                if (i < 3 && current == self.board[i + 1][j]) ||
                   (j < 3 && current == self.board[i][j + 1]) {
                    return false;
                }
            }
        }
        
        true
    }

    pub fn count_empty_cells(&self) -> usize {
        self.empty_mask.count_ones() as usize
    }

    pub fn get_max_tile(&self) -> u32 {
        self.max_tile
    }

    pub fn get_score(&self) -> u32 {
        self.board.iter().flatten().sum()
    }

    pub fn add_random_tile_self(&mut self) {
        Self::add_random_tile(&mut self.board);
        // Update cached values
        self.empty_mask = Self::calculate_empty_mask(&self.board);
        self.max_tile = Self::calculate_max_tile(&self.board);
    }
}

// AI module
pub mod ai {
    use super::*;

    impl GameBoard {
        pub fn calculate_adaptive_depth(&self) -> u32 {
            let empty_cells = self.count_empty_cells();
            let max_tile = self.get_max_tile();
            
            // Much more aggressive depth calculation for higher tiles
            let base_depth = match empty_cells {
                0..=2 => 6,  // very few empty cells, search deeper
                3..=5 => 7,  // moderate empty cells
                6..=8 => 8,  // many empty cells, can search deeper
                _ => 9,      // lots of empty cells, maximum depth
            };
            
            // Increase depth significantly when we have high-value tiles
            if max_tile >= 512 {
                base_depth + 3
            } else if max_tile >= 256 {
                base_depth + 2
            } else if max_tile >= 128 {
                base_depth + 1
            } else {
                base_depth
            }
        }

        pub fn evaluate_board(&self) -> f32 {
            let hash = self.board_hash();
            
            // Check transposition table first
            if let Some(&cached_score) = TRANSPOSITION_TABLE.lock().unwrap().get(&hash) {
                return cached_score;
            }

            let mut score = 0.0;
            
            // Snake pattern weights - prefer tiles in snake-like pattern
            let snake_weights = [
                [20.0, 19.0, 18.0, 17.0],
                [12.0, 13.0, 14.0, 15.0],
                [11.0, 10.0, 9.0,  8.0],
                [0.0,  1.0,  2.0,  3.0],
            ];
            
            // Find highest tile and its position
            let mut highest_tile = 0;
            let mut highest_pos = (0, 0);
            
            for i in 0..4 {
                for j in 0..4 {
                    let value = self.board[i][j];
                    if value > highest_tile {
                        highest_tile = value;
                        highest_pos = (i, j);
                    }
                    if value > 0 {
                        score += value as f32 * snake_weights[i][j];
                    }
                }
            }
            
            // Very strong corner bonus for highest tile (prefer top-left corner)
            let corner_bonus = if highest_pos == (0, 0) {
                highest_tile as f32 * 8.0  // Top-left corner is best
            } else if (highest_pos.0 == 0 || highest_pos.0 == 3) && 
                      (highest_pos.1 == 0 || highest_pos.1 == 3) {
                highest_tile as f32 * 4.0  // Other corners are good
            } else {
                -(highest_tile as f32 * 2.0) // heavy penalty for not being in corner
            };
            score += corner_bonus;
            
            // bonus for empty cells (logarithmic scaling)
            let empty_cells = self.count_empty_cells() as f32;
            let empty_bonus = if empty_cells > 0.0 {
                empty_cells.ln() * 500.0
            } else {
                -5000.0 // very heavy penalty for full board
            };
            score += empty_bonus;
            
            // bonus for smoothness (adjacent tiles with similar values)
            score += self.calculate_smoothness() * 25.0;
            
            // bonus for monotonicity (tiles in increasing/decreasing order)
            score += self.calculate_monotonicity() * 30.0;
            
            // bonus for edge alignment
            score += self.calculate_edge_bonus();
            
            // bonus for merge potential
            score += self.calculate_merge_potential() * 20.0;
            
            // bonus for snake pattern adherence
            score += self.calculate_snake_pattern() * 40.0;
            
            // bonus for keeping high tiles together
            score += self.calculate_clustering() * 25.0;
            
            // penalty for isolated high tiles
            score -= self.calculate_isolation_penalty() * 15.0;
            
            // Cache the result
            TRANSPOSITION_TABLE.lock().unwrap().insert(hash, score);
            score
        }

        fn calculate_snake_pattern(&self) -> f32 {
            let mut score = 0.0;
            
            // Check if tiles follow a snake pattern (top-left to bottom-right)
            let snake_path = [
                (0, 0), (0, 1), (0, 2), (0, 3),
                (1, 3), (1, 2), (1, 1), (1, 0),
                (2, 0), (2, 1), (2, 2), (2, 3),
                (3, 3), (3, 2), (3, 1), (3, 0)
            ];
            
            for (idx, &(i, j)) in snake_path.iter().enumerate() {
                let value = self.board[i][j] as f32;
                if value > 0.0 {
                    // Bonus for tiles in snake order
                    score += value * (16 - idx) as f32;
                }
            }
            
            score
        }

        fn calculate_clustering(&self) -> f32 {
            let mut score = 0.0;
            
            // Bonus for keeping similar tiles close together
            for i in 0..4 {
                for j in 0..4 {
                    if self.board[i][j] != 0 {
                        let value = self.board[i][j];
                        
                        // Check adjacent cells for similar values
                        let adjacent_positions = [
                            (i.wrapping_sub(1), j),
                            (i + 1, j),
                            (i, j.wrapping_sub(1)),
                            (i, j + 1),
                        ];
                        
                        for (ni, nj) in adjacent_positions {
                            if ni < 4 && nj < 4 {
                                let neighbor = self.board[ni][nj];
                                if neighbor == value || neighbor == value / 2 || neighbor == value * 2 {
                                    score += value as f32 * 0.5;
                                }
                            }
                        }
                    }
                }
            }
            
            score
        }

        fn calculate_isolation_penalty(&self) -> f32 {
            let mut penalty = 0.0;
            
            // Penalty for high tiles that are isolated
            for i in 0..4 {
                for j in 0..4 {
                    let value = self.board[i][j];
                    if value >= 64 {
                        let mut isolated = true;
                        
                        // Check if tile has adjacent tiles
                        let adjacent_positions = [
                            (i.wrapping_sub(1), j),
                            (i + 1, j),
                            (i, j.wrapping_sub(1)),
                            (i, j + 1),
                        ];
                        
                        for (ni, nj) in adjacent_positions {
                            if ni < 4 && nj < 4 && self.board[ni][nj] != 0 {
                                isolated = false;
                                break;
                            }
                        }
                        
                        if isolated {
                            penalty += value as f32;
                        }
                    }
                }
            }
            
            penalty
        }

        fn calculate_monotonicity(&self) -> f32 {
            let mut monotonicity = 0.0;
            
            // check horizontal monotonicity
            for i in 0..4 {
                let mut current = 0;
                let mut next = current + 1;
                let mut current_direction = 0;
                let mut score = 0.0;
                
                while next < 4 {
                    while next < 4 && self.board[i][next] == 0 {
                        next += 1;
                    }
                    if next >= 4 {
                        break;
                    }
                    
                    let current_value = self.board[i][current] as f32;
                    let next_value = self.board[i][next] as f32;
                    
                    if current_value > next_value {
                        if current_direction > 0 {
                            score = 0.0;
                        } else if current_direction < 0 {
                            score += next_value;
                        } else {
                            score = next_value;
                        }
                        current_direction = -1;
                    } else if next_value > current_value {
                        if current_direction < 0 {
                            score = 0.0;
                        } else if current_direction > 0 {
                            score += current_value;
                        } else {
                            score = current_value;
                        }
                        current_direction = 1;
                    }
                    
                    current = next;
                    next += 1;
                }
                
                monotonicity += score;
            }
            
            // check vertical monotonicity
            for j in 0..4 {
                let mut current = 0;
                let mut next = current + 1;
                let mut current_direction = 0;
                let mut score = 0.0;
                
                while next < 4 {
                    while next < 4 && self.board[next][j] == 0 {
                        next += 1;
                    }
                    if next >= 4 {
                        break;
                    }
                    
                    let current_value = self.board[current][j] as f32;
                    let next_value = self.board[next][j] as f32;
                    
                    if current_value > next_value {
                        if current_direction > 0 {
                            score = 0.0;
                        } else if current_direction < 0 {
                            score += next_value;
                        } else {
                            score = next_value;
                        }
                        current_direction = -1;
                    } else if next_value > current_value {
                        if current_direction < 0 {
                            score = 0.0;
                        } else if current_direction > 0 {
                            score += current_value;
                        } else {
                            score = current_value;
                        }
                        current_direction = 1;
                    }
                    
                    current = next;
                    next += 1;
                }
                
                monotonicity += score;
            }
            
            monotonicity
        }

        fn calculate_smoothness(&self) -> f32 {
            let mut smoothness = 0.0;
            
            for i in 0..4 {
                for j in 0..4 {
                    if self.board[i][j] != 0 {
                        let current_value = self.board[i][j] as f32;
                        
                        // check right neighbor
                        if j < 3 && self.board[i][j + 1] != 0 {
                            smoothness -= (current_value - self.board[i][j + 1] as f32).abs();
                        }
                        
                        // check bottom neighbor
                        if i < 3 && self.board[i + 1][j] != 0 {
                            smoothness -= (current_value - self.board[i + 1][j] as f32).abs();
                        }
                    }
                }
            }
            
            smoothness
        }

        fn calculate_edge_bonus(&self) -> f32 {
            let mut bonus = 0.0;
            
            for i in 0..4 {
                for j in 0..4 {
                    let value = self.board[i][j] as f32;
                    if value >= 32.0 {
                        // bonus for being on edge
                        if i == 0 || i == 3 || j == 0 || j == 3 {
                            bonus += value * 0.2;
                        }
                        // extra bonus for corners
                        if (i == 0 || i == 3) && (j == 0 || j == 3) {
                            bonus += value * 0.3;
                        }
                    }
                }
            }
            
            bonus
        }

        fn calculate_merge_potential(&self) -> f32 {
            let mut potential = 0.0;
            
            for i in 0..4 {
                for j in 0..4 {
                    if self.board[i][j] != 0 {
                        let value = self.board[i][j];
                        
                        // check adjacent cells for merge potential
                        let adjacent_positions = [
                            (i.wrapping_sub(1), j),
                            (i + 1, j),
                            (i, j.wrapping_sub(1)),
                            (i, j + 1),
                        ];
                        
                        for (ni, nj) in adjacent_positions {
                            if ni < 4 && nj < 4 && self.board[ni][nj] == value {
                                potential += value as f32;
                            }
                        }
                    }
                }
            }
            
            potential
        }

        pub fn find_best_move(&mut self) -> Option<Direction> {
            let depth = self.calculate_adaptive_depth();
            let directions = [Direction::Up, Direction::Down, Direction::Left, Direction::Right];

            // First pass: quick evaluation to order moves
            let mut move_scores: Vec<(Direction, f32)> = directions.iter()
                .filter_map(|&direction| {
                    let mut new_board = self.clone();
                    if new_board.move_tiles(direction) {
                        // Quick shallow evaluation for move ordering
                        let quick_score = new_board.evaluate_board();
                        
                        // Bonus for moves that keep highest tile in corner
                        let highest_tile = new_board.get_max_tile();
                        let mut corner_bonus = 0.0;
                        for i in 0..4 {
                            for j in 0..4 {
                                if new_board.board[i][j] == highest_tile {
                                    if (i == 0 || i == 3) && (j == 0 || j == 3) {
                                        corner_bonus += highest_tile as f32 * 0.2;
                                    }
                                }
                            }
                        }
                        
                        // Bonus for moves that create merges
                        let merge_bonus = new_board.calculate_merge_potential() * 0.1;
                        
                        // Bonus for moves that keep board open
                        let empty_bonus = new_board.count_empty_cells() as f32 * 10.0;
                        
                        Some((direction, quick_score + corner_bonus + merge_bonus + empty_bonus))
                    } else {
                        None
                    }
                })
                .collect();

            if move_scores.is_empty() {
                return None;
            }

            // Sort moves by quick evaluation (best first)
            move_scores.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

            // Second pass: full evaluation with move ordering
            let results: Vec<(Direction, f32)> = move_scores.into_iter()
                .map(|(direction, _)| {
                    let mut new_board = self.clone();
                    new_board.move_tiles(direction);
                    let score = new_board.expectimax(depth - 1, false, f32::NEG_INFINITY, f32::INFINITY);
                    (direction, score)
                })
                .collect();

            // select best move
            let (best_move, _) = results.into_iter()
                .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
                .unwrap();
                
            Some(best_move)
        }

        // Optimized expectimax with alpha-beta pruning and early termination
        fn expectimax(&mut self, depth: u32, is_maximizing: bool, alpha: f32, beta: f32) -> f32 {
            // Early termination conditions
            if depth == 0 {
                return self.evaluate_board();
            }
            
            if self.is_game_over() {
                return -100000.0; // Very heavy penalty for game over
            }
            
            // Check transposition table
            let hash = self.board_hash();
            if let Some(&cached_score) = TRANSPOSITION_TABLE.lock().unwrap().get(&hash) {
                return cached_score;
            }
            
            if is_maximizing {
                let mut best_score = f32::NEG_INFINITY;
                let mut alpha = alpha;
                let directions = [Direction::Up, Direction::Down, Direction::Left, Direction::Right];
                
                for &direction in &directions {
                    let mut new_board = self.clone();
                    if new_board.move_tiles(direction) {
                        let score = new_board.expectimax(depth - 1, false, alpha, beta);
                        best_score = best_score.max(score);
                        alpha = alpha.max(score);
                        
                        // Alpha-beta pruning
                        if alpha >= beta {
                            break;
                        }
                    }
                }
                
                if best_score == f32::NEG_INFINITY {
                    return self.evaluate_board();
                }
                
                best_score
            } else {
                // computer's turn (add random tile)
                let empty_cells = self.get_empty_cells();
                
                if empty_cells.is_empty() {
                    return self.evaluate_board();
                }
                
                let mut total_score = 0.0;
                
                // Consider more empty cells for better accuracy
                let cells_to_consider = if empty_cells.len() > 16 {
                    &empty_cells[..16] // Increased from 12 to 16
                } else {
                    &empty_cells
                };
                
                // weighted expectation: 90% chance of 2, 10% chance of 4
                for &(i, j) in cells_to_consider {
                    // try placing a 2
                    let mut new_board_2 = self.clone();
                    new_board_2.board[i][j] = 2;
                    let score_2 = new_board_2.expectimax(depth - 1, true, alpha, beta);
                    total_score += score_2 * 0.9;
                    
                    // try placing a 4
                    let mut new_board_4 = self.clone();
                    new_board_4.board[i][j] = 4;
                    let score_4 = new_board_4.expectimax(depth - 1, true, alpha, beta);
                    total_score += score_4 * 0.1;
                }
                
                let avg_score = total_score / cells_to_consider.len() as f32;
                
                // Cache the result
                TRANSPOSITION_TABLE.lock().unwrap().insert(hash, avg_score);
                avg_score
            }
        }

        // Optimized empty cells calculation using cached mask
        fn get_empty_cells(&self) -> Vec<(usize, usize)> {
            let mut cells = Vec::new();
            for i in 0..4 {
                for j in 0..4 {
                    if (self.empty_mask & (1 << (i * 4 + j))) != 0 {
                        cells.push((i, j));
                    }
                }
            }
            cells
        }
    }
}

// Web module
pub mod web; 